...
function initHorizontalScroll() {
  ...
  function checkScroll() {
    ...
    if (sectionTop < 0) {
      ...
      const maxTranslate = (panelCount - 1) * stepPx;
      track.style.transform = `translateX(-${maxTranslate}px)`;
      updateDots(panelCount - 1);
      revealPanelContent(panelStates[panelCount - 1]);
    } else {
      ...
      const currentIdx = Math.min(panelCount - 1, Math.max(0, Math.round(progress * (panelCount - 1))));
      updateDots(currentIdx);

      panelStates.forEach((state, i) => {
        if (Math.abs(i - currentIdx) <= 1) {
          revealPanelContent(state);
        }
      });
    }
  }
  ...
  dots.forEach((dot) => {
    dot.addEventListener('click', (e) => {
      ...
      const targetScroll = sectionTop + (panelIndex / (panelCount - 1)) * totalScrollable;
