...
export default function ContributionHeatmap({ data }: ContributionHeatmapProps) {
  if (!data || !data.weeks || data.weeks.length === 0) {
    return (
      <div className="w-full text-center py-10 px-6 bg-night-900/50 rounded-xl border border-neon-cyan/20">
        <div className="w-12 h-12 mx-auto mb-4 text-neon-cyan opacity-90 flex items-center justify-center rounded-full bg-neon-cyan/10 border border-neon-cyan/30">
        ...
        <h4 className="text-lg font-bold text-white mb-2">Live GitHub GraphQL Data</h4>
        <p className="text-gray-400 text-sm max-w-md mx-auto mb-4 leading-relaxed">
          To display live contribution calendar data for <span className="text-neon-cyan font-mono">@Tusharchauhan72</span> via GitHub's GraphQL API, set your <code className="bg-night-800 text-neon-yellow px-2 py-0.5 rounded font-mono text-xs">GITHUB_TOKEN</code> in your environment (<code className="bg-night-800 text-gray-300 px-1.5 py-0.5 rounded font-mono text-xs">.env</code>).
        </p>
        <a
          href="https://github.com/Tusharchauhan72"
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 text-xs text-neon-pink hover:underline font-mono"
        >
